...
// capitol 3
/**
 * @brief Calculates the value of the Ackermann function recursively.
 * 
 * @param m The first parameter of the Ackermann function.
 * @param n The second parameter of the Ackermann function.
 * @return The value of the Ackermann function for the given parameters.
 */
...
double estimeazaPi(int numarDrepte, int inceputDrepte, int numarIncercari, double eroareTolerata)
{
    int totalTraversari = 0; // Totalul numărului de traversări ale acului peste o linie
    int totalAruncari = 0;   // Numărul total de aruncări ale acului

    for (int i = 0; i < numarIncercari; i++)
    {
        int traversari = 0; // Numărul de traversări ale acului peste o linie într-o încercare

        for (int j = 0; j < numarDrepte; j++)
        {
            double pozitieAleatoare = static_cast<double>(rand()) / RAND_MAX; // Generăm o poziție aleatoare pe axa x

            if (pozitieAleatoare < inceputDrepte / static_cast<double>(numarDrepte))
            {
                traversari++; // Dacă poziția aleatoare este mai mică decât proporția dintre inceputDrepte și numarDrepte, acul traversează o linie
            }
        }

        totalTraversari += traversari;
        totalAruncari += numarDrepte;

        double estimarePi = fabs(totalAruncari / static_cast<double>(totalTraversari) - 1 / M_PI); // Estimarea valorii lui pi

        if (estimarePi < eroareTolerata)
        {
            break; // Ieșim din buclă dacă estimarea este în interiorul erorii tolerante
        }
    }

    double estimarePi = totalAruncari / static_cast<double>(totalTraversari);
    return estimarePi;
}
...
void capitol3()
{
    ...
    do
    {
        ...
        case 2:
            ...
            int m, nr;
            cin >> m >> nr;
            cout << ackermann_recursiv(m, nr) << endl;
            cout << endl;
            break;
        case 3:
        {
        ...
        case 4:
        {
            ...
            factoriPrimiRecursiv(nr2, factoriPrimi, count);

            cout << "Factorii primi ai numărului " << nr2 << " sunt: ";
...
void capitol4()
{
    ...
    int *a; // declare a pointer to int outside the switch statement
    do
    {
...
void capitol7()
{
    ...
    do
    {
        ...
        case 0:
            ...
            break;
        default:
            cout << "Optiune invalida cap 7" << endl;
            break;
        }
    } while (opt7 != 0);
}
...
void capitol8()
{
    ...
    do
    {
        ...
        cout << "     8.3 Aproximare integrala --not-done--" << endl;
        cout << "     8.4 Exemplu de alg Monte-Carlo: eliminarea nodurilor intr-un graf --not-done--" << endl;
        cout << "     8.5 Exemplu de alg Las Vegas: problema reginelor (rezolvare probabilista) --not-done--" << endl;
        cout << "0. Iesire din cap8 \n\n";
        cout << "Optiunea meniu 8: ";
        cin >> opt8;
        switch (opt8)
        {
        case 1:
        {
            int numarDrepte, inceputDrepte, numarIncercari;
            double eroareTolerata;

            cout << "Numarul de drepte: ";
            cin >> numarDrepte;

            cout << "Numarul de la care incep dreptele paralele: ";
            cin >> inceputDrepte;

            cout << "Numarul maxim de incercari: ";
            cin >> numarIncercari;

            cout << "Eroarea tolerata: ";
            cin >> eroareTolerata;

            double estimarePi = estimeazaPi(numarDrepte, inceputDrepte, numarIncercari, eroareTolerata);
            cout << "Estimarea valorii lui pi: " << estimarePi << endl;
        }
        break;
        case 2:
            break;
...
void capitol9()
{
    ...
    do
    {
        ...
        cout << "     9.1. Maximul unei functii cu valori reale" << endl;
        cout << "     9.0. Iesire din cap8 \n\n";
        cout << "Optiunea meniu 9: ";
        cin >> opt9;
        switch (opt9)
        {
        case 1:
